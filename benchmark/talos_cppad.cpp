...
void build_bipedal_running_action_models(boost::shared_ptr<crocoddyl::ActionModelAbstractTpl<Scalar> >& runningActionModel) {
...
template <typename Scalar>
void build_bipedal_terminal_action_models(boost::shared_ptr<crocoddyl::ActionModelAbstractTpl<Scalar> >& terminalActionModel) {
	typedef typename crocoddyl::MathBaseTpl<Scalar>::VectorXs VectorXs;
	typedef typename crocoddyl::DifferentialActionModelFreeFwdDynamicsTpl<Scalar>
	  DifferentialActionModelFreeFwdDynamics;
	typedef typename crocoddyl::IntegratedActionModelEulerTpl<Scalar> IntegratedActionModelEuler;
...
int main(int argc, char* argv[]) {
    ...
    boost::shared_ptr<crocoddyl::ActionModelAbstract > terminalModelD;
    boost::shared_ptr<crocoddyl::ActionModelAbstractTpl<ADScalar> > terminalModelAD;
    
    std::vector<boost::shared_ptr<crocoddyl::ActionModelAbstract > > runningModelsCG;
    std::vector<boost::shared_ptr<crocoddyl::ActionModelAbstract > > runningModelsD;
    
    // Build terminal models 
    build_bipedal_terminal_action_models(terminalModelAD);
    build_bipedal_terminal_action_models(terminalModelD);
    ...
    for (size_t i = 0; i < horizon_length; i++) {
		boost::shared_ptr<crocoddyl::ActionModelAbstract > runningModelD;
		boost::shared_ptr<crocoddyl::ActionModelAbstractTpl<ADScalar> > runningModelAD;

		build_bipedal_running_action_models(runningModelD);
		build_bipedal_running_action_models(runningModelAD);
		...
		runningModelsCG.push_back(runningModelCG);
		runningModelsD.push_back(runningModelD);
	}
    
    // Create random initial trajectories
    Eigen::VectorXd x_rand = runningModelsD[0]->get_state()->rand();
    Eigen::VectorXd u_rand = Eigen::VectorXd::Random(runningModelsD[0]->get_nu());
