...
class CostModelCoMVelocityTpl : public CostModelAbstractTpl<_Scalar> {
 public:
  ...
  typedef CostDataCoMVelocityTpl<Scalar> Data;
  ...
  /**
   * @brief Compute the CoM velocity cost
   *
   * @param[in] data  CoM velocity cost data
  ...
  /**
   * @brief Compute the derivatives of the CoM velocity cost
   *
   * @param[in] data  CoM velocity cost data
   * @param[in] x     State point \f$\mathbf{x}\in\mathbb{R}^{ndx}\f$
   * @param[in] u     Control input \f$\mathbf{u}\in\mathbb{R}^{nu}\f$
  ...
  DEPRECATED("Use set_reference<MathBaseTpl<Scalar>::Vector3s>()", void set_vref(const Vector3s& vref_in));
  DEPRECATED("Use get_reference<MathBaseTpl<Scalar>::Vector3s>()", const Vector3s& get_vref() const);
...
template <typename _Scalar>
struct CostDataCoMVelocityTpl : public CostDataAbstractTpl<_Scalar> {
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  CostDataCoMVelocityTpl(Model<Scalar>* const model, DataCollectorAbstract* const data)
      : Base(model, data),
        Arr_Jcom(3, model->get_state()->get_nv()),
        Arr_vcom_dq(3, model->get_state()->get_nv()),
        vcom_dq(3, model->get_state()->get_nv()) {
    Arr_Jcom.setZero();
    vcom_dq.setZero();
